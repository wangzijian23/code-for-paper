...
data = pandas.read_excel('D:/王子健\大三/遥感原理与方法二/@gl/处理1/输出excel/金盏/归一化结果.xlsx')
X = data.iloc[:, 0:7]
Y = data.iloc[:, 7]
x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size=0.3, random_state=0)  # 自动生成数据
regr = ExtraTreesRegressor(n_estimators=700, min_samples_split=5, min_samples_leaf=1, bootstrap=False, max_depth=39)
regr = regr.fit(x_train, y_train)   # 训练训练数据集
Y_test_p = regr.predict(x_test)
feature_importances_ = regr.feature_importances_  # 重要程度

y_pred = Y_test_p * -10219299.85
y_true = y_test.iloc[:].values * -10219299.85
...
mtklpred = mtklpred * -10219299.85
np.savetxt("jinmtkl.xlsx", mtklpred)
